//! Ferro micro-endpoints app — benchmark harness
...
#[derive(Parser)]
#[command(name = "ferro-micro")]
#[command(about = "Ferro micro-endpoints benchmark app")]
struct Cli {
    #[command(subcommand)]
...
async fn show_migration_status() {
    ...
    let db = get_database_connection().await;
    Migrator::status(&db).await.expect("Failed to get migration status");
}

async fn rollback_migrations(steps: u32) {
    println!("Rolling back {} migration(s)...", steps);
    let db = get_database_connection().await;
    Migrator::down(&db, Some(steps)).await.expect("Failed to rollback migrations");
