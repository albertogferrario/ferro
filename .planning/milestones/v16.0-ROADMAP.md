...
- 🚧 **v16.0 Write-Boundary AX — StateMachine-Derived Executor** — Phases 231-232 (active, scoped 2026-06-16). Eliminate the "declare twice" duplication on the projection write path: derive a default write executor (state read → guard re-eval → transition → persist) from the `ServiceDef` StateMachine `ferro-projections` already owns, with an override hook for the app-specific 20% (side effects, related-record writes), sync-by-construction (no executor↔StateMachine drift), driving writes from one declaration across the v15.0 MCP write path and the visual/form write path. Replaces the hand-written `WriteDispatcher` `match`. Coherence-bound: derive FROM the StateMachine, no parallel imperative control surface, no new crate.
...
## 🚧 v16.0 Write-Boundary AX — StateMachine-Derived Executor (Phases 231–232, scoped 2026-06-16)

**Milestone Goal:** Close the last load-bearing gap in the projection/intent killer feature's WRITE path. The READ path is complete — visual (`JsonUiRenderer`), text (`ferro-text::TextRenderer`), and MCP (`ferro-mcp-server::McpRenderer`) all project the same `ServiceDef`, and Phase 213 closed the render-content gaps. The WRITE path still re-imports the imperative surface projection was meant to eliminate: an `ActionDef` whose `transition_trigger` names a `StateMachine` transition is declared in the `ServiceDef`, then the *same* transition facts are re-encoded by hand in a `WriteDispatcher` `match` arm — the workflow is declared twice, with nothing keeping the two in sync. v16.0 derives a default write executor (state read → guard re-evaluation → transition → persist) from the `StateMachine` the framework already knows, adds an override hook for the app-specific 20% (side effects, related-record writes, custom post-transition logic), prevents executor↔StateMachine drift by construction, and drives writes from the single `ServiceDef` across both existing write surfaces (the v15.0 MCP write dispatch and the visual/form path).

**Killer feature:** a state-transition write declared *only* by naming a `StateMachine` transition on the `ActionDef` dispatches through a framework-generated executor with **no hand-written `match` re-encoding the transition** — the "declared twice, fell out of sync" bug class is eliminated, and one declaration backs writes in every modality.

**Source:** `.planning/REQUIREMENTS.md` (EXEC-01..05). Grounded against code 2026-06-16 at workspace 0.2.65: `ferro-projections` has zero executor-derivation machinery (`state.rs` `StateMachine`/`Transition` and `action.rs` `ActionDef.transition_trigger` are purely declarative; `lib.rs` exports no `Executor`/`derive_default_executor`). The hand-written `WriteDispatcher` in the consumer/`ferro-mcp-server` write path (v15.0 `AMCP-04`) is the duplication to eliminate and the immediate downstream consumer of the derived executor.

**Coherence constraint (binding):** the derived executor reads FROM the existing `StateMachine` / `ActionDef` declarations. It must NOT introduce a parallel imperative control surface, a second source of truth for transitions, or a new crate — derivation lives in `ferro-projections` (it owns `StateMachine`/`ActionDef`); consumers call it. (See `feedback_no_duplicate_control_surface`.)

**Out of scope:** deriving a default executor for plain CRUD writes with no `StateMachine` (deferred until the transition-backed path is proven); the operating-AX side (NL description quality ≡ classification accuracy, gated on a funded COMP-03 live run); the projection `body` slot; gestiscilo / consumer-app adoption (a consumer-repo follow-up per `feedback_cross_repo_phase_split`).
...
- [x] **Phase 232: Single-Source Write Surfaces — Wire the Derived Executor, Retire the Hand-Written WriteDispatcher** — route both existing write surfaces through the Phase 231 derived executor: replace the hand-written `WriteDispatcher` `match` in the v15.0 MCP write path (`AMCP-04`) and wire the visual/form write path to the same derived executor, so one `ServiceDef` declaration backs writes in every modality with no per-channel executor. Covers EXEC-05. (completed 2026-06-16)

| Phase | Plans Complete | Status | Completed |
|-------|----------------|--------|-----------|
| 231. StateMachine-Derived Executor (derivation + guard + override + sync) | 2/2 | Complete   | 2026-06-16 |
| 232. Single-Source Write Surfaces (wire executor, retire WriteDispatcher) | 3/3 | Complete   | 2026-06-16 |

#### Phase Details

### Phase 231: StateMachine-Derived Executor — Derivation, Guard Re-Eval, Override Hook, Sync-by-Construction

**Goal:** A developer declares a state-transition write solely by naming a `StateMachine` transition on the `ActionDef` (`transition_trigger`), and `ferro-projections` derives the default executor — state read → guard re-evaluation → transition → persist — from the `StateMachine` declaration alone, with a guard-fail rejection, an override hook for the app-specific 20%, and build/registration-time rejection of any reference to a transition the `StateMachine` does not declare. No hand-written `match` re-encodes transition facts.

**Depends on:** Nothing new — builds on the shipped, declarative `ferro-projections` surface (`state.rs` `StateMachine`/`Transition`, `action.rs` `ActionDef.transition_trigger`, and the v14.0 `evaluated_guards` guard-evaluation surface).

**Requirements:** EXEC-01 (derive default executor from the `StateMachine`; no `WriteDispatcher` `match` for the common path), EXEC-02 (server-side guard re-evaluation at execution; reject a guard-failing transition), EXEC-03 (override hook for app-specific side effects without replacing the base dispatch; common path stays declaration-only), EXEC-04 (build/registration-time rejection of an `ActionDef`/override referencing an undeclared transition — drift eliminated by construction).

**Success Criteria** (what must be TRUE — developer/agent-observable):
  1. A transition write declared *only* by an `ActionDef.transition_trigger` naming a `StateMachine` transition dispatches through the derived executor (state read → guard check → transition → persist) with **no hand-written `match` arm** — demonstrated by a test that declares such an action and drives it end-to-end with zero per-transition imperative code.
  2. The derived executor re-evaluates the transition's guard server-side at execution time (reusing the `evaluated_guards` surface) and **rejects** a transition whose guard does not hold — an agent/caller cannot drive an illegal transition through the derived path (proven by a guard-false test asserting the write is refused and state is unchanged).
  3. A developer attaches an app-specific side effect (e.g. a related-record write or notification) to a derived executor through the override hook, and it runs **in addition to** the base transition dispatch without re-declaring the transition — the common (no-override) path remains declaration-only.
  4. An `ActionDef` (or override) whose `transition_trigger` names a transition the `StateMachine` does not declare is **rejected at build or registration time, not at runtime** — a fixture referencing a non-existent transition fails to register/build, demonstrating drift is structurally impossible.
  5. The derivation lives entirely in `ferro-projections` (no new crate, no parallel imperative DSL); `StateMachine`/`ActionDef` remain the single declared source of transition facts.
...
**Goal:** The derived executor from Phase 231 drives writes from the single `ServiceDef` across both existing write surfaces — the v15.0 MCP write dispatch (`AMCP-04`) and the visual/form write path — so one declaration backs writes in every modality with no per-channel executor. The hand-written `WriteDispatcher` `match` that re-encoded transition facts is removed.

**Depends on:** Phase 231 (the derived executor + override hook + sync-by-construction surface this phase wires the write surfaces onto).

**Requirements:** EXEC-05 (the derived executor drives writes from the single `ServiceDef` across the MCP write dispatch and the visual/form write path; one declaration backs writes in every modality, no per-channel executor).

**Success Criteria** (what must be TRUE — developer/agent-observable):
  1. The v15.0 MCP write path (`ferro-mcp-server` `AMCP-04`) dispatches a state-transition write through the Phase 231 derived executor; the hand-written `WriteDispatcher` `match` re-encoding the transition is **deleted**, and the MCP write tests pass against the derived path (server-side guard re-evaluation, idempotency, and audit behaviour preserved).
  2. The visual/form write path executes the same state-transition write through the same derived executor — the same `ServiceDef` declaration backs both modalities with no second, channel-specific executor.
  3. A single transition declared once on a `ServiceDef` is exercised through **both** write surfaces in a test, demonstrating identical transition semantics (same guard re-eval, same persist) with no duplicated control surface.
  4. After this phase, grepping the write path shows no hand-authored `match` re-encoding `StateMachine` transitions — transition facts exist only in the `StateMachine` declaration.
...
- [x] 232-03-PLAN.md — Both-channels single-source proof (SC3) + SC4 grep-clean + full workspace gate
