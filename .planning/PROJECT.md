...
**Progress:** Phase 239 (soft-delete substrate — `deleted_at` migration, `resolved_table`/`resolved_soft_delete_column`, `is_server_injected_field`, read-path `deleted_at IS NULL`) and Phase 240 (CRUD input-schema derivation + `list_` query polish — CRUD-01/02/04) complete. Phase 240 added `ServiceDef::is_write_excluded_field` (single shared write-field predicate), `build_create/update/delete_input_schema`, `is_range_filter_field`, and extended `build_input_schema`/read dispatch with `<field>__{gt,gte,lt,lte,ne,in}` + `sort` (equality filters and `limit`/`offset` unchanged). The `create_/update_/delete_` tools are *listed with correct schemas but not yet executable* — calls return a flag-gated `not_yet_implemented` structured envelope; execution (`derive_crud_plan` + `framework::write` wiring) is Phase 241. Remaining: 241 (execution + soft-delete), 242 (write authz + tenant injection + non-disclosure), 243 (app flip + e2e + catalog/docs).
