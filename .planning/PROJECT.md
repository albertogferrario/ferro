...
(shared-transport broadcast fan-out for multi-replica delta delivery) complete 2026-08-14.
Phases 247–249 remain.
...
---
*Last updated: 2026-08-14 — Phase 246.1 (inserted prerequisite) complete: `ferro-broadcast` gained a configurable shared fan-out transport so a delta published in one process reaches subscribers attached to another. A `BroadcastTransport` trait + `BusEnvelope` wire type abstract the bus; an `InMemoryTransport` (tokio broadcast channel) is the default-build CI substitute; a feature-gated `RedisTransport` (redis 0.27 pub/sub, separate PUBLISH/SUBSCRIBE connections, strict drop-on-malformed deserialization) is the real shared bus behind the optional `redis-transport` feature. `Broadcaster` fan-out stamps each envelope with a process-unique `origin_id`, suppresses its own bus echo, delivers remote events via `send_to_channel_local_only` (never re-publishing), fans out only `ServerMessage::Event` (presence/control variants stay per-process), and never fails the caller on a bus error. The default build stays byte-for-byte dependency-free (redis strictly feature-gated); `--all-features` CI is green with no Redis server (live test feature+env gated). Verification 3/3 must-haves; code review 0 critical / 3 warning (WR-01: consumer bootstrap not yet calling `.with_transport` — deferred to Phase 247). The two-OS-process live-Redis deployment proof is tracked as a partial HUMAN-UAT item. Milestone v16.4 now at 244/245/246/246.1 complete; 247–249 remain.*
