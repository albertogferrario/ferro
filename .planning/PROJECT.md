...
---
*Last updated: 2026-05-21 — Phase 177 complete (ferro-reservation kernel atomicity hardening): `ReservationKernel::hold` race condition closed by wrapping the check-then-act sequence in `conn.begin_with_config(IsolationLevel::Serializable, AccessMode::ReadWrite)`; SQLite native single-writer serialization handles the SQLite path, Postgres `SET TRANSACTION ISOLATION LEVEL SERIALIZABLE` with SQLSTATE 40001 → `ReservationError::Insufficient` translation handles the Postgres path; load-bearing SC-1 race test (50 iterations) and SC-5 audit-atomicity test added in `ferro-reservation/tests/concurrent_hold.rs`; cfg-gated Postgres mirror at `ferro-reservation/tests/concurrent_hold_postgres.rs`; `docs/src/database/reservations.md` Consistency Model rewritten (stale `tokio::Mutex` workaround advice removed). All 8 locked decisions D-01—D-08 honored; 6/6 Success Criteria verified for SQLite path; 36 ferro-reservation tests pass. Two human-verification items remain in 177-HUMAN-UAT.md: live Postgres SSI run + gestiscilo-it `concurrent_double_book_same_staff` cross-repo regression. Previous footer: Phase 160 complete (v1 JSON-UI API deletion, 2026-05-17).*
