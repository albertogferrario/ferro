...
## Current Milestone: v16.6 POS Component Suite

**Goal:** Touch-first sale-screen components in the ferro-json-ui builtin catalog — product grid, cart, numpad, quantity/category primitives — at a tablet interaction quality bar, derivable from a `ServiceDef` through the projection layer and agent-authorable through the v16.5 MCP + design-lint boundary. Consumer-paired with gestiscilo's register/counter mode.

**Target features:**
- Sale-screen builtins (product grid/tile, cart panel + line items, numpad, quantity stepper, category navigation) added to the builtin catalog with the established lockstep (drift-guard count, `ferro-base.css` regen, ferro-mcp catalog count).
- Touch-first interaction quality bar (hit targets, density, press states, kinetics) built on the v16.5 token vocabulary — no parallel touch system.
- Projection → register: a `ServiceDef` with products/orders derives a working sale screen within the existing seven-intent vocabulary (no new parallel archetype surface).
- Agent-authoring boundary: catalog + `generation_context` extensions and intent-keyed `design::lint` rules covering POS compositions.
- Single crates.io publish at milestone end; gestiscilo's register phase gates on it.

**Out of scope:** payment flow (cash/card/split), receipt rendering, shift/session close — the sale screen itself only.

**Status:** In progress — Phase 257 complete (2026-07-06): projection → register (POS-10). The projection pipeline derives a working sale screen from a `ServiceDef` under the existing Collect intent: `register_template()` supplies a Collect→Register `IntentSlotTemplate` through the existing `VisualContext.templates` override channel (built-in Collect default stays Form; seven-intent vocabulary and `KNOWN_INTENTS` unchanged), and `emit_register_root()` emits the fill-viewport two-grid composition (Form ancestor, SelectionPanel + confirm, TileGrid with a Tile `$each` template, meaning-driven `$data` bindings) that passes catalog validation and all four register lint rules with zero findings. Builder additions: `ElementBuilder.each(path, as_)` and `Spec::builder().fill_viewport(bool)`; `Catalog::validate` gained an `$each`-template guard so data-bound typed props validate. The `/cassa` sample is projection-derived — `cassa.json` and the `rimuovi` endpoint deleted; integration test asserts 200 + `ferro-fill` chain + rendered product names. Code review 1 critical / 4 warnings — all fixed pre-completion (the critical: handler data not nested under `data`, which rendered an empty tile grid). One human UAT pending (tablet visual quality, 257-HUMAN-UAT.md). Next: Phase 258 (MCP surface + docs + single publish). v16.4 Work Distribution (244–249) remains queued and independent.

## Shipped Milestone: v16.5 JSON-UI Design System (shipped 2026-07-04, 0.2.86)
...
- Phase 256 complete (2026-07-06) — Component renderers + BUILTIN lockstep (milestone v16.6, POS-01/03/04/05/06/09). All five register builtins are first-class catalog members — `TileGrid` (48), `FilterTabs` (49), `QuantityStepper` (50), `Numpad` (51), `SelectionPanel` (52) — each registered in its own commit with both drift-guard counts (catalog.rs canonical + ferro-mcp mirror) bumped in lockstep and the History comment as audit trail. The Tile was redesigned to the operator interaction model (2026-07-05): tap-to-add-only — the tile root is a single ≥44px tap surface carrying `data-qty-inc` (reusing the shipped tiles runtime; no new add-path JS), no on-tile steppers or qty display; new additive `TileProps.price_cents: Option<u64>` emits `data-unit-price` (integer cents), `color` narrowed to `Option<Tone>` with an exhaustive full-literal match (closes the dynamic-class vector); image/stock-badge visuals landed. The un-deferred CartRuntime slice shipped as `runtime/selection.rs` (`setupSelection`): a pure-view reconciler driven by bubbling `input` events from `[data-qty-input]` fields — tile taps, panel steppers, and numpad writes all funnel through one `reconcile()`; lines clone from a server-rendered `<template data-selection-line-template>`, metadata reads from the tile DOM (`data-filter-text`/`data-unit-price`), running total is integer-cents (`formatMoney` display-only), remove-on-zero, EmptyState `style.display` toggle; the form state stays the single source of truth (no `data-cart-target`, 254 D-18 honored). `Grid.row_weights` renders fractional `grid-template-rows` inline style in fill mode only; `initQtyButton` honors `data-qty-min/-max/-step`. Locale audit (D-28): neutral English defaults (`"All"`, `"Total"`, English aria-labels), overridable via additive `all_label`/`total_label` props. Code review 0 critical / 5 warnings — all 5 fixed pre-verification (form_id scope emission, NaN guards, RULE_COMPONENTS TileGrid, overridable labels, Tile docs de-stepper-ed); `ferro-base.css` regenerated; D-30 schema churn confirmed no-op. Verifier: automated checks pass on all 5 roadmap SCs; full CI-exact gate green (753 ferro-json-ui tests). 256-HUMAN-UAT pending (3 items): live reconciler smoke, filter-tab filtering, iOS 16px-input zoom — natural venue is Phase 257 when `/cassa` flips to these components.
- Phase 255 complete (2026-07-05) — Vocabulary neutralization + POS runtime modules + double-submit protection (milestone v16.6, POS-08 + SC-0). Operator decision 2026-07-05: the builtin catalog carries domain-neutral structural nouns only — `ProductTile`→`Tile` (`TileProps`, `product_id`→`item_id`; published-component break documented in the components.md migration table), `ProductGridProps`→`TileGridProps`, `CartPanelProps`→`SelectionPanelProps` (consumer-specific `show_staff`/`show_people` REMOVED), `CategoryNavProps`→`FilterTabsProps`, lint ids `pos-*`→`register-*` (`REGISTER_TRIGGER_TYPES = [TileGrid, SelectionPanel, Numpad]`), `POS_*` touch constants de-prefixed, `data-product-*`→`data-filter-*`; SC-0 zero-hits grep gate closed across ferro-json-ui/ferro-mcp/app/docs; count stays 47. Two new ES5 runtime modules: `runtime/numpad.rs` (`setupNumpad` — tap-surface keypad, quantity + cents-shift price mode with integer-cents hidden-field contract, bubbling `input` dispatch) and `runtime/filters.rs` (`setupFilters` — AND token/text tile filtering via `data-filter-tokens`/`data-filter-text`/`data-filter-scope`, `style.display` toggling, semantic active-tab classes); both drift-list tests extended; dispatcher hardened with per-setup try/catch (review WR-03). Double-submit protection (POS-08): additive `ButtonProps.disable_on_submit` → `data-disable-on-submit`, guard inside `setupFormGuards` bound on the form `submit` event with `e.defaultPrevented` check (WR-02) and `btn._submitted` + `pageshow` bfcache reset; `/cassa` confirm wrapped in a real Form so the guard binds (WR-01); number guard sees `input[data-numpad-input]`; write-kernel.md gained the layered client-guard + `idempotency_key` + PRG section. Verifier 5/5 (SC-0..SC-4); code review 0 critical / 3 warnings — all fixed pre-verification; full CI-exact gate green (783 ferro-json-ui tests). One HUMAN-UAT pending: live /cassa double-submit smoke test in a browser.
