...
---
*Last updated: 2026-05-31 — Phase 181 complete (JSON-UI inline error rendering): Fix A merges runtime data into spec.data clone before resolve_expressions; Fix B writes singular `error` field matching prop shape; Checkbox / CheckboxList / Switch / Input(file) brought to D-06 error-state + ARIA parity; cross-repo audit confirms clean break (no consumer depended on broken plural shape); docs page covers four authoring patterns. Full workspace gate: 2812 tests passing. One human-verification item remains in 181-HUMAN-UAT.md: gestiscilo browser UAT on 5 representative forms. Previous footer: Phase 177 complete (ferro-reservation kernel atomicity hardening, 2026-05-21).*
