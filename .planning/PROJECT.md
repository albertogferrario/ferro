...
- v15.0 through v18.0 shipped, and the entire shipped backlog was archived on 2026-08-12 via `/gsd-complete-milestone` (git tags `v15.0`, `v16.0`–`v16.6`, `v17.0`, `v18.0`). The v17.0 and v18.0 code is published in the crates.io 0.3.x line (`ferro-rs` **0.3.9**). The current milestone is v16.4 Work Distribution (`#[offload]` service methods, Phases 244–249, not yet planned). ~30 workspace crates.

## Shipped Milestone: v18.0 Projection-Native Frontend Substrate (shipped in-tree 2026-07-27)

**Goal:** Derive a custom (Inertia) frontend's data, field schema, and permitted-actions from the same `ServiceDef`/`ActionDef` that already drives the visual and MCP surfaces, delivered Inertia-first in one `Inertia::from_projection` call; the frontend's writes reuse the channel-agnostic `dispatch_write` kernel. The one structural change lifts guard-visibility (`permitted_actions`) out of `ferro-mcp-server` into `framework` so guards are evaluated in exactly one place.

**Delivered (Phase 263, SUBST-01..05, verified 15/15):** a pure `schema_contract(&ServiceDef)` derivation in `ferro-projections`; `permitted_actions` lifted into `framework` and shared by MCP `tools/list` and Inertia (one guard-evaluation site); `Inertia::from_projection` delivering `{ schema, data, permitted_actions }` props, tenant-scoped, on the framework-side Inertia facade; write reuse through `dispatch_write(channel="web")`; single-source parity tests (Inertia↔MCP permitted-actions, write parity, schema snapshot, tenant-scoping). No new crate; publish operator-gated. Anchor spec: `docs/superpowers/specs/2026-07-27-headless-projection-substrate-design.md`.

**Out of scope:** generic JSON delivery (decoupled SPA / native) and projection-native JSX component derivation — later, separate specs.

## Shipped Milestone: v17.0 Live Projection Surface (shipped in-tree 2026-07-26)

**Goal:** Join `ferro-projection`'s per-key snapshot + delta broadcast with `ferro-json-ui`'s server-side rendering into a declarative `LiveFragment` element that binds a rendered fragment to a projection key and re-renders in place on delta — server-authoritative, no WASM.

**Delivered (Phases 259–262, LIVE-01..04):** the `LiveFragment` builtin (its defining capability — first-paint snapshot plus in-place re-render on delta via a no-WASM client runtime that swaps inner HTML; the live DOM swap was verified via Chrome MCP); request-scoped `#[memoize]` (fan-out dedup wired into the projection render pass); a one-line `asset!()` over the existing bundle/pipeline crates; `ferro-mcp` catalog + `generation_context` + docs; single operator-gated publish. No new crate; one per-key snapshot per fragment (list reconciliation deferred). Anchor spec: `docs/superpowers/specs/2026-07-21-live-projection-surface-design.md`.

## Shipped Milestone: v16.6 POS Component Suite (shipped 2026-07-06, 0.2.89)
...
v16.x / v17.0 / v18.0 backlog was archived. Phases 244 (`#[offload]` macro → Job + payload
derivation), 245 (typed result handle + serializable-contract enforcement), and 246
(result → read-model snapshot) complete 2026-08-13; the inserted prerequisite Phase 246.1
(shared-transport broadcast fan-out for multi-replica delta delivery) and Phase 247
(read-model delta → broadcast streaming — race-safe `resolve()`, non-blocking enqueue, redacted
client read-back) complete 2026-08-14. Phases 248–249 remain, plus an appended Phase 249.1
(offload surface convergence — remove the transitional/fallback code paths introduced during
244–249 incremental delivery, per the feature-branch convention).
...
- Phase 248 complete (2026-08-14) — Deployable worker runtime, milestone v16.4 (OFFLOAD-05). The application binary gains a top-level `worker` subcommand with a repeatable `--queue` selector (`clap` `ArgAction::Append`; omit for all registered queues) and `serve` gains `--no-worker` for web-only replicas — both routed through a single shared `run_common_boot(bootstrap_fn, no_worker)` boot seam extracted from the serve path (no forked bootstrap logic), with `run_worker` re-exported at the `ferro::` facade. `Queue::registered_queue_names()` (deterministic `BTreeSet`) makes the in-process serve worker consume every declared queue rather than only `"default"` (D-05). `#[offload(queue = "name")]` now parses an optional queue argument (syn-2 `parse_nested_meta`, unknown args rejected with a clear diagnostic + trybuild snapshot) and threads the declared queue into both the emitted `JobRegistrarEntry.queue` and the `Offloadable::offload()` dispatch (`.on_queue(...)`), so fault-domain routing is declarative on the trait method. WR-01 attaches a shared broadcast transport from configuration at boot under the `redis-transport` feature; feature-off with a URL set warns and falls back to the in-process hub without panic and without logging the URL (D-07). Capacity scales by running more workers — no framework autoscaler (SC#4 structural guard clean; deferred to 2.0 per the milestone scope). Verification 9/10 automated must-haves (the one open item is the multi-process exactly-once + fault-isolation UAT, Manual-Only by design, persisted in `248-HUMAN-UAT.md`); code review 0 critical / 2 warnings — both fixed in-phase (WR-01 worker DB-connect now emits guided remediation + `exit(1)` instead of a bare panic; WR-02 warns when a requested `--queue` has no registered handler). Full CI-exact gate green.
...
---
*Last updated: 2026-08-14 — Phase 248 (deployable worker runtime) complete: the app binary runs as `<app-bin> worker [--queue <name>]` at N replicas and `serve --no-worker` for web-only replicas, both through one shared `run_common_boot` seam (`run_worker` re-exported at the `ferro::` facade). `#[offload(queue = "name")]` threads a declared queue into `JobRegistrarEntry.queue` + `Offloadable::offload().on_queue(...)`; `Queue::registered_queue_names()` makes serve consume all declared queues (D-05). WR-01 boot-time shared-transport attach under `redis-transport`, warn + in-process fallback when the feature is off (D-07, URL never logged). No framework autoscaler (SC#4 clean) — capacity scales by running more workers. Verification 9/10 (multi-process exactly-once UAT is Manual-Only, persisted in `248-HUMAN-UAT.md`); code review 0 critical / 2 warnings resolved in-phase (worker DB-connect guided-error path; unregistered-queue warn). Full CI-exact gate green. Milestone v16.4 now at 244/245/246/246.1/247/248 complete; 249 + 249.1 remain. Prior — Phase 247 (read-model delta → broadcast streaming): the fire-and-forward loop closes end-to-end. The worker result hook persists the snapshot then publishes an `offload.result` delta on `projection.offload.result.{handle}` (persist-before-broadcast; the failed delta carries no `error` field, D-05). `enqueue_and_mark_pending` is the non-blocking request-side entrypoint (writes a `{"status":"pending"}` marker so a read-back distinguishes unknown-handle from in-flight). `resolve()` is the race-safe client await — subscribe-first, read-back-once (short-circuit terminal handles), then await the delta — with `remove_client` on every exit path (the timeout/subscribe-failure leaks WR-01/WR-02 found in review were fixed in-phase, commit `55b00768`). `read_result_redacted` masks raw errors as `"terminal error"` for browser clients (D-10). Integration suite (`framework/tests/offload_delta_broadcast.rs`) proves cross-replica delta, ordering-based non-blocking, and redaction over two `Broadcaster`s on one shared transport; `docs/src/features/queues.md` documents the subscribe-then-await pattern. Verification 3/3; code review 0 critical (2 warnings resolved). Known debt: a persist-only broadcaster `None` fallback in bootstrap and "backward-compatible" framing on `OffloadResult::Pending` were preserved by the executors — captured for removal in the newly-appended Phase 249.1 (offload surface convergence), per the operator's no-backwards-compatibility direction. Milestone v16.4 now at 244/245/246/246.1/247 complete; 248–249 + 249.1 remain.*
