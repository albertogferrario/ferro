...
---
*Last updated: 2026-07-06 — Phase 258 complete; milestone v16.6 POS Component Suite SHIPPED (ferro-rs 0.2.89 + ferro-payments 0.1.6 on crates.io, tag v0.2.89, GitHub Release with binaries). Phase 258 closed the agent-authoring loop for the register: `generation_context` now carries register-composition guidance (when Register vs form-only Collect; the hidden-input form-state contract with single confirm POST; `data-filter-*`/numpad/`data-disable-on-submit` attributes; the fill_viewport app|dashboard constraint; the register lint rules; a pointer to `register_template()`), drift-guarded against the builtin registry, `design::rules()`, and the runtime JS bundle; `json_ui_catalog` BUILDER_API documents `fill_viewport(bool)` + `.each(path, as_)` and RULE_COMPONENTS maps `register-fill-viewport` to Grid/TileGrid/SelectionPanel/Numpad; docs/src gained the five component sections (TileGrid, SelectionPanel, FilterTabs, QuantityStepper, Numpad), the Register Layout Template + fill_viewport chapters, and the builder API additions — mdBook build green. SC-1 (catalog count 52 + names) was recorded as pre-existing Phase 256 evidence. Publish: full CI-exact gate green (fmt/clippy --all-features/test --all-features/doc -D warnings), single workspace bump 0.2.88→0.2.89, master fast-forwarded and pushed, both crates verified live via crates.io API. Code review: 0 critical / 5 advisory warnings (docs example import path, drift-guard coverage hardening, components.md table gaps) — follow-up patch candidates, no factual drift in shipped MCP output. gestiscilo register phase can pin ferro-rs 0.2.89.*
