...
**Consumer pairing:** gestiscilo-it v6.10 Phase 203 consumes `application_fee_for` via the published 0.9.0 bump; gestiscilo Phase 202 (Connect webhook endpoint + secret split) needs no new ferro surface (it consumes the already-shipped 0.8.0 webhook primitives). Auto-publishes via GH Actions on push to master per `feedback_ferro_publish.md`.
...
| 198. Streamable HTTP Endpoint + Unauthenticated Challenge | 1/2 | In Progress|  |
