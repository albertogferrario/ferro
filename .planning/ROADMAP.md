...
Phases 233–236 (PAY-POLY-DM-01..04 / SVC-01..05 / WH-01..06 / REAP-01..04) — full details archived in [milestones/v16.1-ROADMAP.md](milestones/v16.1-ROADMAP.md).

Delivered: a new project-agnostic `ferro-payments` crate — polymorphic `PaymentIntent` + `Billable`/`BillableLoader` traits, webhook `SyncDispatcher` integration with idempotent auto-refund, and `ReleaseExpiredPaymentIntents`/`ReconcileRefundsInFlight` reapers — published as `ferro-payments 0.1.0`, reusing the existing `ferro-stripe` surface. [CONSUMER-PAIRED with gestiscilo 218–223]
...
- [x] **Phase 243: App integration, e2e, envelope guard & catalog/docs** — Flip the app's `order` projection to CRUD, drive create→list→update→delete over `:8090/mcp` and the visual surface, extend the structured-envelope regression guard, update `ferro-mcp` catalog/docs. (completed 2026-06-24)
- [x] **Phase 243.1: Projection computed/derived fields** — Honor the `writable` flag in CRUD write-input derivation (Gate F: read-only fields excluded from `create_`/`update_`), and orchestrate server-side recomputation of a derived field via the existing override-hook registry. Demonstrated end-to-end: `order.total = SUM(line_items.amount)`, recomputed on every line-item write. Surfaced by the Phase 243 live `:8090/mcp` field test. Decimal insert after 243 (follows the CRUD integration it extends; no renumbering of 244–249). Imported plan: `.planning/phases/243.1-projection-computed-derived-fields/243.1-01-PLAN.md`. Design: `docs/superpowers/specs/2026-06-24-projection-computed-derived-fields-design.md`. (completed 2026-06-24)
...
- [x] 243-02-PLAN.md — In-process MCP e2e: create→list→update→delete + per-verb envelope guard + MCP↔visual parity + confirmation flow (CRUD-01/02/03/05/06)
- [x] 243-03-PLAN.md — ferro-mcp code_templates + generation_context + docs/src CRUD opt-in; full BLOCKING workspace gate (SC#4)
...
- `ProductTileProps` + `render_product_tile` + `runtime/product_tiles.rs` (existing, extended here; renamed `Tile`/`TileProps`/`render_tile`/`runtime/tiles.rs` in Phase 255).
...
- Form-state accumulation stays this milestone — Tile hidden inputs accumulate quantities into a single confirm POST (no per-tap server round-trips). REVISED 2026-07-05 (operator): the SelectionPanel is a **live client-side view** of that form state — the CartRuntime slice (line appear/update on tile tap, per-line qty edit + remove, integer-cents running total) is un-deferred into Phase 256. Interaction model: tiles are tap-to-add-only; ALL quantity editing happens in the SelectionPanel (Shopify POS model).
- Domain-neutral vocabulary (operator decision 2026-07-05): the builtin catalog carries structural nouns only — `Tile`, `TileGrid`, `SelectionPanel`, `FilterTabs` — mirroring the structural intent vocabulary. Commerce naming is confined to sample apps (`/cassa`) and docs examples; no consumer-specific props in any `ferro-*` crate. Renames applied in Phase 255 (all pre-publish; `ProductTile`→`Tile` is a break of the published component, documented in the consumer migration table).
...
**Anchor research:** `.planning/research/` (STACK, FEATURES, ARCHITECTURE, PITFALLS, SUMMARY — 2026-07-04). Open decisions resolved (2026-07-04, operator): form-state cart only — CartRuntime DEFERRED (against the synthesis recommendation) **[SUPERSEDED 2026-07-05: CartRuntime slice un-deferred into Phase 256 — see scope constraints above; the STACK.md Cart Runtime sketch is back in scope]**; FilterTabs (née CategoryNav) is a standalone builtin per POS-03; Grid row_weights included (POS-09); Numpad included; barcode keyboard-wedge deferred.
...
- [x] **Phase 258: MCP Surface + Docs + Publish** — `json_ui_catalog` count + component names updated; `generation_context` register composition guidance; `docs/src` updates; single crates.io publish. Delivers POS-12, POS-13. (completed 2026-07-06)
...
**Goal:** The catalog vocabulary is domain-neutral (operator decision 2026-07-05) and the POS runtime modules (numpad input, token/text tile filtering) are in the bundle with a stable data-attribute contract before any render function targets it; the double-submit guard is in place for selection-mutation forms. Scope boundary: NO cart-state JS — quantities accumulate in Tile hidden inputs (existing tiles-runtime contract, `data-qty-*` unchanged) and submit as a single confirm POST; the live CartRuntime is deferred.

**Depends on:** Phase 254 (`ProductTileProps` data attributes defined; touch foundation established — both renamed here).

**Requirements:** POS-08

**Success Criteria** (what must be TRUE):
0. Vocabulary neutralization is complete: `ProductTile`→`Tile` (component type, `TileProps`, `render_tile`, `runtime/tiles.rs`/`setupTiles`, `product_id`→`item_id`), `ProductGridProps`→`TileGridProps`, `CartPanelProps`→`SelectionPanelProps` (with `show_staff`/`show_people` consumer-specific props REMOVED), `CategoryNavProps`→`FilterTabsProps`, lint ids `pos-fill-viewport`/`pos-grid-fill`/`pos-cart-present`→`register-fill-viewport`/`register-grid-fill`/`register-selection-present`, `POS_*` constants renamed without the domain prefix; `grep -rn 'ProductTile\|product_tile\|setupProductTiles\|data-product-\|CartPanel\|CategoryNav\|ProductGrid' ferro-json-ui/src ferro-mcp/src app/src docs/src` returns zero hits; `docs/src/json-ui/components.md` migration table documents the `ProductTile`→`Tile` break; component count stays 47 (rename, not addition).
1. `bundle_contains_all_setup_functions` test passes for `setupNumpad` and `setupFilters` — both function names are present in the emitted IIFE source.
2. `dispatcher_invokes_every_setup` test passes — `ferroRuntime()` calls `setupNumpad()` and `setupFilters()` exactly once; both are no-ops when their elements are absent.
...
**Goal:** All five new POS builtins are first-class catalog members; a spec author can compose a complete sale screen from `TileGrid`, `SelectionPanel`, `FilterTabs`, `QuantityStepper`, and `Numpad`; `Grid` `row_weights` renders asymmetric layout.
...
2. Every interactive element in `TileGrid`, `FilterTabs`, `SelectionPanel`, `QuantityStepper`, and `Numpad` renders with `min-h-[44px]` enforced at Rust render time (preferred `min-h-[56px]` for Numpad keys); HTML assertions verify by element inspection.
...
**Plans:** 5/5 plans complete
- [x] 256-01-PLAN.md — Props substrate (price_cents, color→Tone, SelectionPanel currency) + Tile tap-to-add redesign + Grid row_weights
- [x] 256-02-PLAN.md — Register TileGrid + FilterTabs (shared tab-strip helper, integrated strip, search, SC-5)
- [x] 256-03-PLAN.md — Register QuantityStepper + Numpad + SelectionPanel render (count → 52)
- [x] 256-04-PLAN.md — SelectionPanel live reconciler runtime (selection.rs) + initQtyButton bounds + bundle wiring
- [x] 256-05-PLAN.md — ferro-base.css regen + tap-to-add migration note + schema-export verify + full gate
**UI hint**: yes
...
**Depends on:** Phase 256 (`TileGrid` and `SelectionPanel` must be in `BUILTIN_TYPES` before `catalog_validate` accepts projector output referencing them).
...
- [x] 257-04-PLAN.md — [gap closure] FormProps.fill height-chain through the Form layer so the register SelectionPanel footer pins under fill_viewport (256 D-15)
**UI hint**: yes
...
**Success Criteria** (what must be TRUE):
1. `json_ui_catalog` returns all five new component names (TileGrid, SelectionPanel, FilterTabs, QuantityStepper, Numpad); `test_all_components_present` in `ferro-mcp` passes at the updated count (52); both count assertions agree.
2. `generation_context` includes register composition guidance: when to use the Register layout template vs. a form-only Collect spec; the form-state selection contract (`data-qty-input` hidden-input accumulation, single confirm POST) and the filter/numpad data attributes (`data-filter-tokens`, numpad target field); the `fill_viewport` dependency for register layouts; the `design::lint` rules agents should check.
...
- [x] 258-03-PLAN.md — verify /cassa flip stands; full CI-exact gate; operator-gated single publish 0.2.89 (+ ferro-payments 0.1.6 rider); gestiscilo handoff brief [POS-13]
**UI hint**: yes
...
| Requirement | Phase |
|-------------|-------|
| POS-01 (TileGrid builtin with touch-first grid + search) | Phase 256 |
| POS-02 (Tile additive props, backward-compat; delivered 254 as ProductTile, renamed 255) | Phase 254 |
| POS-03 (FilterTabs standalone builtin, ≥44px targets) | Phase 256 |
| POS-04 (SelectionPanel builtin with qty stepper + EmptyState) | Phase 256 |
...
**Plans:** 4/4 plans complete
  ...
  - Wave 2: [ ] `261-04-PLAN.md` — `asset!()` proc-macro + `ferro::asset!` re-export + trybuild pass fixture
