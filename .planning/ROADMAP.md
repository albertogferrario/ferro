...
- [ ] 154-02-PLAN.md — Register crate in workspace (Cargo.toml members + version bump 0.2.31→0.2.32, publish.yml WAVE1B_CRATES, CLAUDE.md, README.md)
- [ ] 154-03-PLAN.md — SeaORM migration CreateReservationsTable + entity Model (12 columns + 2 composite indexes) + lib.rs entity re-exports
- [ ] 154-04-PLAN.md — Leaf-type bodies: Resource trait, ReservationContext builder, ReservationEvent + ReleaseReason (serde + Event impl), ReservationHandle serde tests
- [ ] 154-05-PLAN.md — ReservationKernel<R> with hold/commit/release/extend (GuardedUpdate + AuditEntry + dispatch ordering) + 7 unit tests covering D-47-1..7
- [ ] 154-06-PLAN.md — run_sweep_once + concurrent_hold integration (D-48) + proptest property tests (D-49) + cross-crate integration (D-50)
