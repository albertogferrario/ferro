...
**Plans:** 5 plans (Wave 1: 01, 02, 03 parallel · Wave 2: 04 · Wave 3: 05)

Plans:
- [ ] 246-01-PLAN.md — ferro-projection direct snapshot write/read API (snapshot_write/snapshot_read) + unit tests (D-01/02/03)
- [ ] 246-02-PLAN.md — framework offload facade: persist_result/persist_error/read_result + OffloadResult<T> + ferro-projection dep + ::ferro::offload::* wiring (D-07/11/13)
...
## ✅ v16.6 POS Component Suite (Phases 254–258) — Shipped 2026-07-06 (0.2.89) [CONSUMER-PAIRED with gestiscilo register/counter mode]

POS-01..13 — full details archived in [milestones/v16.6-ROADMAP.md](milestones/v16.6-ROADMAP.md); milestone audit PASSED ([milestones/v16.6-MILESTONE-AUDIT.md](milestones/v16.6-MILESTONE-AUDIT.md)).

Delivered: five touch-first sale-screen builtins (TileGrid, SelectionPanel, FilterTabs, QuantityStepper, Numpad) at a tablet quality bar, a live client-side SelectionPanel over form state (tap-to-add tiles, per-line qty edit, integer-cents total, single confirm POST), and a `Register` layout template under the Collect intent. Domain-neutral catalog vocabulary; no new crate.

## ✅ v17.0 Live Projection Surface (Phases 259–262) — Shipped 2026-07-26

LIVE-01..04 — full details archived in [milestones/v17.0-ROADMAP.md](milestones/v17.0-ROADMAP.md).

Delivered: a declarative `LiveFragment` element binds a rendered fragment to a `ferro-projection` per-key snapshot and re-renders in place on delta — server-authoritative, no WASM (live DOM swap verified via Chrome MCP). Plus request-scoped `#[memoize]` (render-path fetch dedup) and a one-line `asset!()` over the existing bundle/pipeline crates. No new crate; single operator-gated publish at Phase 262.

---

## ✅ v18.0 Projection-Native Frontend Substrate (Phase 263) — Shipped in-tree 2026-07-27 (publish operator-gated)

SUBST-01..05 — full details archived in [milestones/v18.0-ROADMAP.md](milestones/v18.0-ROADMAP.md).

Delivered: derive a custom (Inertia) frontend's data, field schema, and permitted-actions from the same `ServiceDef`/`ActionDef` that drives the visual and MCP surfaces, delivered in one `Inertia::from_projection` call; writes reuse `dispatch_write(channel="web")`. The one structural change lifts `permitted_actions` from `ferro-mcp-server` into `framework` — guards evaluated in exactly one place. Phase 263 verified 15/15; dogfood `/orders` page tracked.
