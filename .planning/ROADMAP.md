...
Plans:
- [ ] 180-01-PLAN.md — Runtime types (`ActionError`, `ActionOk`, `ActionResult`, `IntoActionError`, `handle_action_result`) in `framework/src/http/action.rs` + re-exports
- [ ] 180-02-PLAN.md — Extract param-extraction helpers from `ferro-macros/src/handler.rs` into `ferro-macros/src/utils.rs` as `pub(crate)`
