...
- 🔧 **v7.4 Security Hardening** — Phases 72-74
...
#### Phase 72: [SEC] Binary Response Type (C1)

**Goal**: Add `HttpResponse::bytes(Vec<u8>)` to serve raw binary data (images, PDFs, downloads) without requiring `unsafe { String::from_utf8_unchecked }`. Currently apps must use unsafe to serve non-UTF-8 file content through the text-based response API.
**Audit ref**: C1 — `unsafe` UB on arbitrary file bytes in mkmenu storage and QR controllers
**Plans**: TBD

Plans:
- [ ] TBD (run /gsd:plan-phase 72 to break down)

#### Phase 73: [SEC] Security Headers Middleware (M6) — COMPLETE

**Goal**: Add a `SecurityHeaders` middleware that sets `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Content-Security-Policy`, and `Strict-Transport-Security` with sensible defaults. Register it in the default middleware stack generated by `ferro new`.
**Audit ref**: M6 — no CSP, X-Content-Type-Options, or X-Frame-Options in mkmenu
...
Plans:
- [x] 73-01: SecurityHeaders middleware with OWASP defaults and builder API
- [x] 73-02: CLI bootstrap template integration and documentation

#### Phase 74: [SEC] Session Absolute Expiry (L2)

**Goal**: Add `created_at` column to session table migration template. Enforce absolute session lifetime (configurable, default 30 days) in addition to idle timeout. Provide `Session::invalidate_all_for_user()` for password change flows.
**Audit ref**: L2 — no absolute session expiry, no invalidation hook
**Plans**: TBD
...
| v7.4 Security Hardening | 72-74 | 3/TBD | In Progress | - |

**Total: 14 milestones shipped, 152 plans.**
