...
| 192. MCP Template + Docs | 1/2 | In Progress|  |
...
**Success Criteria** (what must be TRUE):
  ...
  2. The `From<stripe::Event> for StripeChargeRefunded` parser populates `refund_id` from `stripe::EventObject::Refund(r) => Some(r.id.to_string())`; returns `None` when the event payload's object is not a `Refund` (defensive — should never happen for `charge.refunded` events but guards against malformed payloads)
...
**Plans**: TBD (1-2 plans expected; the field addition + fixture updates + version bump fit a single plan; the CHANGELOG + manual-publish checkpoint may split out).
