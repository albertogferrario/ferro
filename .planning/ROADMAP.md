...
- 🚧 **v11.7 Tailwind Static CSS Pipeline** — Phase 143 (INSERTED, urgent). Replace `@tailwindcss/browser@4` runtime JIT (dev-only, fails on Safari/WebKit) with a build-time Tailwind CLI step producing a pre-built `ferro-base.css`. Flip `JsonUiConfig::tailwind_cdn` default to `false`, add `stylesheet_urls` field, serve base CSS from a framework-owned route. Theme overrides move from `<style type="text/tailwindcss">` magic MIME to plain CSS variable overrides. Source: gestiscilo-it Safari field report 2026-04-20. [Context](phases/143-tailwind-static-css-pipeline/143-CONTEXT.md)
...
### 🚧 v11.7 Tailwind Static CSS Pipeline (Urgent — Phase 143)

**Milestone Goal:** Replace Tailwind v4's in-browser JIT runtime with a build-time CSS pipeline. The runtime (`@tailwindcss/browser@4`) is dev-only per Tailwind's own docs, downloads a WebAssembly blob, and fails silently on Safari/WebKit (observed: gestiscilo.it login page renders as unstyled HTML — "raw text" — to every Safari user). Chrome's permissive engine masked the problem. Fix is to pre-build a base stylesheet in ferro, ship it as a static asset, and flip `JsonUiConfig::tailwind_cdn` default to `false`.

**Source:** gestiscilo-it production field report, 2026-04-20.

**What changes:**
- `ferro-json-ui` ships `ferro-base.css` pre-compiled via Tailwind v4 standalone CLI (Rust-native, no Node). Contains every utility class any ferro-json-ui component can emit, plus a default theme declaring the full variable vocabulary (surface/role/shape/shadow/typography tokens).
- `JsonUiConfig::tailwind_cdn` default: `false`. Keep the flag for opt-in dev use.
- `JsonUiConfig::stylesheet_urls: Vec<String>` added — default `["<framework-served base CSS URL>"]`. `<link rel="stylesheet">` tags emitted in order.
- Framework registers a static route serving the embedded `ferro-base.css` bytes.
- Theme injection changes: replace `<style type="text/tailwindcss">{theme.css}</style>` (magic MIME consumed by runtime only) with a plain `<style>` containing CSS variable overrides. Apps override appearance by writing `:root { --color-primary: ...; }` and `.dark { ... }` — no Tailwind recompile needed on the app side.
- Scaffolder (`ferro-cli`): scaffolded apps link the framework-served base CSS, no CDN script.

**What stays:**
- Component rendering logic (render.rs unchanged).
- Theme loading mechanism in ferro-theme (theme files still declare the dark/light vars; just emitted as plain CSS rather than Tailwind-processed CSS).
- Bunny Fonts `<link>` preload (orthogonal, fonts only).
- `JsonUiConfig::tailwind_cdn(true)` as an explicit opt-in for dev ergonomics — not default.

**Success criteria:**
1. `curl https://<ferro-app>/` in Safari (desktop + iOS) renders fully styled immediately — no FOUC, no WASM, no third-party script for CSS.
2. `JsonUiConfig::default()` response contains `<link rel="stylesheet" href="...">` and no `<script src=".../tailwindcss/browser...">`.
3. All existing JsonUi pages visually identical to pre-change snapshot in Chrome, Safari, Firefox (gestiscilo login / dashboard / public `/s/{slug}/` pages are the reference).
4. Theme overrides (gestiscilo's dark theme) continue to work; `body class="dark bg-background text-text font-sans"` renders correctly.
5. No network request to `cdn.jsdelivr.net` on any ferro page.

**Key risks:**
1. **Utility coverage** (MEDIUM): the CLI input glob must cover every file that emits Tailwind classes in ferro-json-ui + framework. Missed files → missing utilities → visual regressions. Mitigation: scan all `.rs` files in both crates; add a CI test asserting the generated CSS contains a known-tricky set of classes.
2. **Theme vocabulary lock-in** (LOW): once `ferro-base.css` is built with a fixed token set, apps cannot introduce new tokens and get new utilities without a ferro rebuild. Acceptable for v1 — apps override values, not token names. Document as a constraint.
3. **Breaking change for CDN-dependent apps** (LOW): current ferro consumer is gestiscilo-it only; no external consumers pre-1.0.
