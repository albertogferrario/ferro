...
status: complete
stopped_at: v16.6 POS Component Suite archived 2026-08-12 (tag v16.6; PASSED milestone audit moved to milestones/). v16.1/16.2/16.3/16.5/16.6 done — no open items (all verifications passed, audit passed). Remaining backlog: v17.0, then current+complete v18.0.
last_updated: "2026-08-12T17:00:00.000Z"
...
**v16.3 MCP CRUD Data Surface** (tag `v16.3`), **v16.5 JSON-UI Design System** (tag `v16.5`;
Phase 252 CLI-formatting UAT re-confirmed live), and **v16.6 POS Component Suite** (tag `v16.6`;
PASSED milestone audit moved to milestones/) — requirements sections extracted from the accreted
REQUIREMENTS.md; all archived 2026-08-12. (The earlier claim that v16.0 was a backlog item was
...
|-----------|--------|-------|
| v17.0 Live Projection Surface | 259–262 | **next to archive**; phases complete 2026-07-26; publish operator-gated; requirements NOT in REQUIREMENTS.md (reconstruct from ROADMAP); **verify Phase 260 LiveFragment Chrome-MCP swap is closed before archiving** |
...
- Phase 246.1 inserted after Phase 246 (2026-08-12): **Shared-transport broadcast fan-out for multi-replica delta delivery** (INSERTED, prerequisite for Phase 247). `ferro-broadcast` depends only on `tokio` + `tokio-tungstenite` — an in-process hub, so a delta published by a worker on one replica is never observed by a client socketed on another. This breaks v16.4's fire-and-forward result path at any scale-out deployment, making a shared fan-out transport (Redis pub/sub, Postgres `LISTEN`/`NOTIFY`, or equivalent) a prerequisite rather than an independent improvement. Transport selected by configuration; the in-process hub stays the default for single-node and development. Phase 247 `Depends on` updated to 246 + 246.1; OFFLOAD-04 now maps to both. Anchor spec updated in the same pass (`docs/superpowers/specs/2026-06-24-offload-work-distribution-design.md`, commit `f6cb7a90`), which also records the Phase 248 worker shape: the worker is the application's own binary under `<bin> worker --queue <class>`, not a `ferro` CLI subcommand, since job handlers live in the app crate; `serve` keeps its in-process worker and gains `--no-worker`; deploy scaffolding emits worker components from `[package.metadata.ferro.deploy]` rather than `[[bin]]` detection. **v16.4 remains queued — it activates only after v18.0 completes; the milestone pointer is not flipped here.**
- Phases 259-262 added (2026-07-21): v17.0 Live Projection Surface roadmap created. 4 phases: **259** Request-scoped memoization — `MemoStore` in request extensions + `#[memoize]` attribute (fan-out dedup + concurrent-call coalescing) wired into the `ServiceDef → IntentGraph` render pass so N intents over one key issue one fetch (LIVE-01); **260** Live reactive fragment — `LiveFragment` builtin element binds a child template to a `ferro-projection` per-key snapshot, server re-renders on delta and pushes HTML on the existing `projection.{name}.{key}` channel, a small no-WASM client runtime swaps inner HTML; ONE binding pattern, list-diffing deferred (LIVE-02, killer feature); **261** `asset!()` ergonomics — one-line embed + content-type infer + `ferro-bundle` hashed-URL registration, opt-in `ferro assets fetch` for Iconify/Fontsource on the Rust toolchain alone (LIVE-03); **262** MCP + catalog + docs + publish — `LiveFragment` through the full drift-guard lockstep, `generation_context` for all three, `docs/src`, single operator-gated publish (LIVE-04). LIVE-01..04 mapped 259..262 (1:1); single publish at Phase 262; no mid-milestone publishes. **No new crates** — memoize in framework+ferro-macros, live fragment in ferro-json-ui+ferro-projection, asset macro over the existing ferro-assets/ferro-bundle. Substrate already exists (ferro-projection delta broadcast, ferro-bundle content-hashing, Request extensions type-map); this milestone is the authoring primitives on top. Anchor spec: `docs/superpowers/specs/2026-07-21-live-projection-surface-design.md`. Created via brainstorm (not full `/gsd-new-milestone`: v16.6 complete but pending archive, numbering continues; independent of the queued v16.4 244–249). Repo-neutral framing (no external-framework references per neutral-repo-voice rule). Next: `/gsd-plan-phase 259`. Prereq consideration: v16.6 archive (`/gsd-complete-milestone v16.6`) and current-milestone pointer flip are operator decisions, not done here.
...
Last session: 2026-08-12 (v16.1 + v16.2 + v16.3 + v16.5 + v16.6 milestone closes)
Stopped at: v16.1/v16.2/v16.3/v16.5/v16.6 archived (tags v16.1..v16.3, v16.5, v16.6). REQUIREMENTS.md is an accreted multi-milestone file — extract each section at close (done for v16.3/v16.5/v16.6; only v16.4 QUEUED + v18.0 CURRENT remain in it). v16.6 audit moved to milestones/. Continuing oldest-first.
Resume file: None
Next action: `/gsd-complete-milestone v17.0` (next in backlog). Remaining: v17.0, then current+complete v18.0. **v17.0 (Live Projection Surface, 259–262) has NO section in REQUIREMENTS.md — reconstruct requirements from ROADMAP inline (like v16.1/v16.2). Before archiving v17.0, verify Phase 260 LiveFragment Chrome-MCP browser-swap is closed** (memory flagged it as possibly pending). Then v18.0 (current): extract its section from REQUIREMENTS.md; no milestone audit exists. Local commits/tags v16.1/16.2/16.3/16.5/16.6 not yet pushed. ferro-cli debug binary built (target/debug/ferro). gestiscilo register phase unblocked on ferro-rs 0.2.89.
