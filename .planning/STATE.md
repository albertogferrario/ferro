...
- Phase 181 added (2026-05-29): `ferro-static-template` — new generic primitive for rendering a directory of HTML files through MiniJinja with partial composition (`{% extends %}` / `{% include %}` / `{% block %}` / `{% macro %}`). Wires `set_loader(path_loader(dir))` (gating: needs the `loader` feature on the `minijinja` dep), enforces the `_`-prefix partial convention (files/dirs starting with `_` are loadable but never rendered as standalone pages), exposes `render_directory`, `strip_partials`, `is_partial_path`. Path traversal sandboxed via `path_loader`. Driver: gestiscilo-it field test — published frontends (jetskiadriatic, hairfactory) have ~80 lines of identical `<head>` per page and no way to compose. Source spec lives in gestiscilo-it/app/docs/superpowers/specs/2026-05-29-frontend-template-composition-design.md. Paired with gestiscilo Phase 173 (consumer + jetski refactor).
...
Last session: 2026-05-20T23:54:35.123Z
