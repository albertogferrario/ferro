...
| 253-02-T1 | 02 | 1 | DS-07 catalog design_system vocabulary + per-component guidance | T-253-04 | variant/tone/size derived from canonical enums (no duplicate); component_guidance from RULE_COMPONENTS bidirectionally drift-guarded (mapped ids in registry, registry ids mapped, component names in builtin set); 47-count mirror intact | unit | `cargo test -p ferro-mcp json_ui_catalog` | ❌ Wave 1 | pending |
