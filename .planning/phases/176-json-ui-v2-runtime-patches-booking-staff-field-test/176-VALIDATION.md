...
| 176-01-03 | 01 | 1 | F7 omit-empty | `Card { badge: None }` serializes without the key | unit | `cargo test -p ferro-json-ui card_props_omits_empty_badge` | ❌ W0 | ⬜ pending |
| 176-01-04 | 01 | 1 | F7 render | `render_card` with `badge="B"` emits a Badge-styled span containing "B" | unit | `cargo test -p ferro-json-ui render_card_emits_badge_slot` | ❌ W0 | ⬜ pending |
...
| 176-01-07 | 01 | 1 | F8 omit-empty | `Card { subtitle: None }` serializes without the key | unit | `cargo test -p ferro-json-ui card_props_omits_empty_subtitle` | ❌ W0 | ⬜ pending |
| 176-01-08 | 01 | 1 | F8 render | `render_card` with `subtitle="S"` emits muted-text element containing "S" beneath title | unit | `cargo test -p ferro-json-ui render_card_emits_subtitle_slot` | ❌ W0 | ⬜ pending |
| 176-01-09 | 01 | 1 | F7+F8 ordering | `render_card` emits subtitle BEFORE description and AFTER title; badge in title row | unit | `cargo test -p ferro-json-ui render_card_slot_order_title_subtitle_description` | ❌ W0 | ⬜ pending |
...
| 176-02-01 | 02 | 1 | F9 reproduction | Construct minimal spec mirroring consumer chip-strip Grid with `visible: {path:"/has_staff", operator:"eq", value:true}` and `data.has_staff=true`; assert render output contains the Grid + children | unit | `cargo test -p ferro-json-ui grid_visible_true_renders_grid_and_children` | ❌ W0 | ⬜ pending |
| 176-02-02 | 02 | 1 | F9 negative | Same spec with `data.has_staff=false`; assert render output omits the Grid entirely | unit | `cargo test -p ferro-json-ui grid_visible_false_omits_grid` | ❌ W0 | ⬜ pending |
...
- [ ] `ferro-json-ui/src/component.rs` (tests module) — `card_props_schema_includes_badge`, `card_props_schema_includes_subtitle`, `card_props_round_trips_badge`, `card_props_round_trips_subtitle`, `card_props_omits_empty_badge`, `card_props_omits_empty_subtitle`
- [ ] `ferro-json-ui/src/render/containers.rs` (tests module) — `render_card_emits_badge_slot`, `render_card_emits_subtitle_slot`, `render_card_slot_order_title_subtitle_description`, `grid_visible_true_renders_grid_and_children`, `grid_visible_false_omits_grid`
