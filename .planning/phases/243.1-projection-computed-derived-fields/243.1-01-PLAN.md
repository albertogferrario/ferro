...
must_haves:
  truths:
    - "create_order and update_order MCP input schemas exclude the read-only `total` field"
    - "Creating an order with no line items yields total = 0"
    - "order.total equals SUM(line_items.amount) after line-item create/update"
    - "Deleting a line item recomputes order.total (read-your-writes)"
  artifacts:
    - "ferro-projections/src/service.rs — Gate F in is_write_excluded_field (honors writable)"
    - "app/src/projections/line_item.rs — line_item CRUD projection"
    - "app/src/controllers/mcp.rs — recompute_order_total_hook via with_override"
    - "app/src/tests/computed_total_e2e.rs — in-process e2e field test"
---

<!--
Imported from docs/superpowers/plans/2026-06-24-projection-computed-derived-fields.md
Design spec: docs/superpowers/specs/2026-06-24-projection-computed-derived-fields-design.md
Task bodies below carry the full TDD steps; the frontmatter above is the GSD must-haves contract.
-->
