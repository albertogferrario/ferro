...
**ferro intent test:** `cargo test -p ferro-projections --test catalog staff_browse_intent`

## Source (gestiscilo repo)

- Controller (before): `src/controllers/staff/list.rs`
- View JSON (before, deleted after migration): `src/views/staff/list.json`
- Backing model: `src/models/entities/staff.rs`
...
4. **Intent confirmation:** `derive_intents(&service)[0].intent == Browse` — [ ] PASS (ref: ferro test `staff_browse_intent`)
