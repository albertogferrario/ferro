...
| SUBST-03 | `Inertia::from_projection` serializes `{ schema, data, permitted_actions }` props correctly | unit | `cargo test -p framework --features "projections inertia" from_projection` | `framework/src/inertia/projection.rs` (cycle-forced placement — see PATTERNS correction) | ⬜ pending |
