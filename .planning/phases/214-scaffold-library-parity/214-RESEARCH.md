...
1. **`Request::db()` method existence**
   - What we know: The Inertia scaffold templates call `req.db()` but `pub fn db()` was not found on `Request` in `framework/src/`.
   - What's unclear: Either it exists (the grep missed it) or the Inertia scaffold templates have an additional bug outside the 52 errors (the benchmark used `--api`, which bypasses Inertia templates).
   - Recommendation: The implementer reads `framework/src/http/request.rs` early in the implementation. If `req.db()` does not exist, the Inertia template has a secondary bug that must be fixed in the same phase.

2. **`make:auth` users model generation**
   - What we know: `auth_controller_template` imports `crate::models::users`. The migration alters a users table. We did not confirm whether a `users.rs` model file is emitted.
   - What's unclear: The complete `make:auth` command output.
   - Recommendation: The implementer reads `ferro-cli/src/commands/auth.rs` (if it exists) and the `models/mod.rs.tpl` before fixing D-05. If no model is emitted, `make:auth` must emit one, or the template must reference a user type that exists without a separate model file.

3. **`#[rule]` in API `form_fields`**
   - What we know: The API `{form_fields}` slot is generated by the scaffold command. We did not verify whether `#[rule(…)]` attributes are emitted in the API path.
   - What's unclear: Whether `form_fields` in the API context contains `#[rule]` annotations.
   - Recommendation: The implementer greps the scaffold command's field generation logic for the `--api` path to confirm. If yes, add `ValidateRules` to the API template's form struct derive.
