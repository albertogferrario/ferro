...
//!
//! The lifecycle assertions (2–4) are gated `#[cfg(not(feature = "confirmation"))]`
//! matching `crud_e2e.rs`'s cycle test; the schema assertion (1) needs no gate.
...
mod tests {
    ...
    // ── Fixture helpers (copied from crud_e2e.rs) ────────────────────────────
    ...
    /// Seed two tenants and their users into `db`.
    ///
    ...
    // ── Call helpers (adapted — use exposed_services() + make_write_dispatcher()) ──

    /// Invoke a CRUD write tool through `handle_tools_call` with an authorized
    /// `McpContext`.
    ///
    /// Uses `crate::controllers::mcp::exposed_services()` (order + line_item) and
    /// `crate::controllers::mcp::make_write_dispatcher()` (carries the recompute
    /// hook) so the hook fires on every line-item write.
    async fn call_write(
    ...
    /// Invoke a read tool through `handle_tools_call`.
    ///
    /// Uses `crate::controllers::mcp::exposed_services()` (order + line_item) and
    /// a noop dispatcher (reads don't go through the write kernel).
    async fn call_read(
        db: &DatabaseConnection,
        tool_name: &str,
        arguments: ferro::serde_json::Value,
    ...
    ///
    /// No feature gate needed — this is a static schema check.
    #[tokio::test]
    ...
    ///
    /// - Truth 2: creating an order with no line items yields total == 0.
    /// - Truth 3: order.total == SUM(line_items.amount) after two creates.
    /// - Truth 4: deleting one line item recomputes order.total (read-your-writes).
    ///
    /// Gated `not(feature = "confirmation")`: delete_line_item is destructive and
    /// uses the two-step confirm flow when the feature is on.
