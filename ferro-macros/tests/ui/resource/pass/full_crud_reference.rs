...
//!
//! Exercises `resource_get` + `resource_post` + `TenantScoped` + `validate_or_redirect` on the
//! same model via the `ferro::` facade path. Proves CRUD-06: the four phase artifacts compose
//! into one coherent CRUD surface a downstream crate can import.
//!
...
use ferro::{
    async_trait, resource_get, resource_post, ActionResult, Request, Response, TenantContext,
    TenantScoped,
};
